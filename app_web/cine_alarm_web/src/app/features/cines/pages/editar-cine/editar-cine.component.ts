...
@Component({
  ...
  // Reutiliza los estilos del formulario de nuevo cine
  styleUrl: '../nuevo-cine/nuevo-cine.component.css',
})
export class EditarCineComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const cine = this.cineService.getCinePorId(id);
    ...
    this.cineId = cine.id;
    this.nombreOriginal = cine.nombre;

    // Llenar el formulario con los datos actuales
    this.form.setValue({
      nombre: cine.nombre,
      locacion: cine.locacion,
      formatos: cine.formatos.split(',').map(f => f.trim()).filter(f => f),
      peliculas: cine.peliculas.map(p => p.id),
    });
  }
